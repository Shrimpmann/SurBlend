...
def test_update_ingredient(client: TestClient, db: Session, auth_headers):
    ...
    # Create ingredient
    ingredient = Ingredient(
        name="Original Name", code="ORIG", type=IngredientType.DRY, cost_per_ton=500.00
    )
    db.add(ingredient)
    db.commit()
    ...
    # Update it
    update_data = {"name": "Updated Name", "cost_per_ton": 600.00}

    response = client.put(
        f"/api/ingredients/{ingredient.id}", json=update_data, headers=auth_headers
    )
...
def test_delete_ingredient(client: TestClient, db: Session, auth_headers):
    ...
    # Create ingredient
    ingredient = Ingredient(
        name="To Delete", code="DELETE", type=IngredientType.DRY, cost_per_ton=100.00
    )
    db.add(ingredient)
    db.commit()
    db.refresh(ingredient)

    response = client.delete(f"/api/ingredients/{ingredient.id}", headers=auth_headers)
...
Imported DAP,IMP-DAP,dry,18,46,0,685"""
    ...
    response = client.post("/api/ingredients/import", files=files, headers=auth_headers)
