...
blend_chemicals = Table(
    "blend_chemicals",
    Base.metadata,
    Column("blend_id", Integer, ForeignKey("blends.id")),
    Column("chemical_id", Integer, ForeignKey("chemicals.id")),
    Column("ai_percentage", Float, nullable=False),
    Column("amount", Float, nullable=False),
)
...
class User(Base):
    ...
    tags = relationship("Tag", back_populates="created_by_user")
...
class Ingredient(Base):
    ...
    chlorine = Column(DECIMAL(5, 2), default=0)  # Added Cl
...
class Chemical(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(100), unique=True, nullable=False)
    ai_percentage = Column(DECIMAL(5, 2), nullable=False)  # Active Ingredient %
...
class Customer(Base):
    ...
    # Preferences
    default_margin_type = Column(String(20), default="percent")
    ...
    tags = relationship("Tag", back_populates="customer")
...
class Blend(Base):
    ...
    target_s = Column(DECIMAL(5, 2))
    target_ca = Column(DECIMAL(5, 2))
    target_mg = Column(DECIMAL(5, 2))
    target_fe = Column(DECIMAL(5, 2))
    target_zn = Column(DECIMAL(5, 2))
    target_mn = Column(DECIMAL(5, 2))
    target_b = Column(DECIMAL(5, 2))
    target_cl = Column(DECIMAL(5, 2))
    ...
    chemicals = relationship("Chemical", secondary=blend_chemicals)
    quotes = relationship("Quote", back_populates="blend")
    tags = relationship("Tag", back_populates="blend")
...
class Quote(Base):
    ...
    margin_value = Column(DECIMAL(5, 2))
...
class Tag(Base):
    ...
    ingredients = Column(JSON)  # For manual entries: [{name: str, quantity: float}]
    created_by = Column(Integer, ForeignKey("users.id"))
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
    ...
    blend = relationship("Blend", back_populates="tags")
    created_by_user = relationship("User", back_populates="tags")
